    ...
    const newLines = Array.from({ length: lineCount }, (_, lineIndex) => {
      ...
      return {
        ...
        dots: Array.from({ length: dotCount }, () => ({
          ...
          // Updated particle colors to Elegant Gold and Off-white
          color: Math.random() > 0.5 ? '#E8C591' : 'rgba(255, 255, 255, 0.4)',
      ...
      >
        {lines.map((line, lineIndex) => (
          <motion.g key={lineIndex} variants={lineVariants}>
            {/* Vertical line - Updated color to transparent Deep Royal Blue */}
            <line
              x1={`${line.x}%`}
              y1="0"
              x2={`${line.x}%`}
